...
# TODO: FIGURE OUT WHETHER TO USE VELOCITIES ARE RAW PRICES
...
def main():
    ...
    for i in range(start_len // 2):
        multivar_reg = {col: multivariate_regression(df, col) for col in cols}
        mn_col = min(multivar_reg, key=multivar_reg.get)
        print(f"{mn_col}: {multivar_reg[mn_col]}")
        cols.remove(mn_col)

    print("Remaining columns:", cols)
    multivar_reg = pd.DataFrame({col: [multivariate_regression(df, col)] for col in cols})
